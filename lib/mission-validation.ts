...
export function validateMissionCode(rawCode: string, missionId = 1): MissionResult {
  ...
  if (missionId === 2) {
    ...
    if (!variableMatch) return { status: "invalid", steps: 0, message: "energy 변수에 숫자를 저장하고 다음 줄에서 charge(energy)를 실행해 보세요." };
    const energy = Number(variableMatch[1]);
    if (energy === 5) return { status: "success", steps: 3, message: "충전 완료! energy 변수에 5를 정확히 저장했어요." };
    if (energy < 5) return { status: "too-short", steps: Math.min(energy, 2), message: `에너지가 ${5 - energy}만큼 부족해요. 목표는 5예요.` };
    return { status: "too-far", steps: 3, message: `에너지가 ${energy - 5}만큼 넘쳤어요. 정확히 5로 맞춰 보세요.` };
  }
  ...
  if (missionId === 3) {
    ...
    return { status: "invalid", steps: 0, message: "door_open이 True일 때 if 안에서 move()를 실행해야 해요. 들여쓰기도 확인해 보세요." };
  }
